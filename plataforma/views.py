...
def pacientes(request):
    if request.method == "GET":
        ...
        if (len(nome.strip()) == 0
            ...
            messages.add_message(request, constants.ERROR, 'Complete todos los campos')
            return redirect('/pacientes/')

        if not idade.isnumeric():
            messages.add_message(request, constants.ERROR, 'Introduce una edad válida')
            return redirect('/pacientes/')

        pacientes = Pacientes.objects.filter(email=email)
        if pacientes.exists():
            messages.add_message(request, constants.ERROR, 'Ya hay un familiar con este correo electrónico')
            return redirect('/pacientes/')
        ...
        try:
            ...
            messages.add_message(request, constants.SUCCESS, 'Paciente registrado exitosamente')
            return redirect('/pacientes/')
        except:
...
def dados_paciente(request, id):
    ...
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Este familiar esta mal')
    ...
    if request.method == "GET":
        ...
        messages.add_message(request, constants.SUCCESS, 'Datos registrados exitosamente')
        return redirect('/dados_paciente/')
...
def plano_alimentar(request, id):
    ...
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Este paciente no es tuyo.')
...
def refeicao(request, id_paciente):
    ...
    if request.method == "POST":
        ...
        messages.add_message(request, constants.SUCCESS, 'Comida Registrada')
...
def opcao(request, id_paciente):
    if request.method == "POST":
        ...
        messages.add_message(request, constants.SUCCESS, 'Opción Registrada')
        return redirect(f'/plano_alimentar/{id_paciente}')